...
# Data directory pattern for neural data
...
################################################################################
# 3) Define Generic CPC Trainer, Evaluator, and Prediction Saver
################################################################################
...
def get_predictions(model, data_loader, device):
    """
    Returns the concatenated latent representations (z) for the entire data_loader.
    """
    model.eval()
    preds_list = []
    with torch.no_grad():
...
def run_self_supervised_experiment():
    # Names of models for saving results.
    ...
    for fish_num in fish_list:
        ...
        # Load neural data (remove the last two columns as in your original code)
        neural_data = np.load(DATA_DIR_PATTERN.format(fish_num), allow_pickle=True)[:, :-2]
        # Transpose to (num_frames, num_neurons)
        ...
        for run_id in range(1, num_runs + 1):
            ...
            # Create a run-specific folder.
            run_folder = os.path.join(fish_save_dir, f"run_{run_id}")
            os.makedirs(run_folder, exist_ok=True)
            
            for seq_len in seq_lengths:
                print(f"\nSequence Length = {seq_len}")
                # Create a folder for this sequence length.
                seq_folder = os.path.join(run_folder, f"seq_{seq_len}")
                ...
                # Save predictions.
                preds = get_predictions(model_gpt2, test_loader, device)
                save_path = os.path.join(seq_folder, f"fish{fish_num}_predictions_GPT2_seq{seq_len}_run{run_id}.npy")
                ...
                preds = get_predictions(model_lstm, test_loader, device)
                save_path = os.path.join(seq_folder, f"fish{fish_num}_predictions_LSTM_seq{seq_len}_run{run_id}.npy")
                np.save(save_path, preds)
                ...
                preds = get_predictions(model_res, test_loader, device)
                save_path = os.path.join(seq_folder, f"fish{fish_num}_predictions_Reservoir_seq{seq_len}_run{run_id}.npy")
                np.save(save_path, preds)
                ...
                try:
                    ...
                    preds = get_predictions(model_deepseek, test_loader, device)
                    save_path = os.path.join(seq_folder, f"fish{fish_num}_predictions_DeepSeek_seq{seq_len}_run{run_id}.npy")
                    np.save(save_path, preds)
                except Exception as e:
                    print("DeepSeek model failed to load/train. Recording NaN for CPC loss and predictions.")
                ...
                preds = get_predictions(model_bert, test_loader, device)
                save_path = os.path.join(seq_folder, f"fish{fish_num}_predictions_BERT_seq{seq_len}_run{run_id}.npy")
                np.save(save_path, preds)
