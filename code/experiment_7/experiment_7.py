...
def rmse_from_mse(mse_vals):                    # convert list of MSE losses → RMSE
...
def train(model, opt, tr_loader, va_loader):
    ...
    for ep in range(num_epochs):
        ...
        if (ep+1) % 10 == 0 or ep == 0:
            print(f"Epoch {ep+1:3d}/{num_epochs} — RMSE:"
                  f" train {np.sqrt(tr_hist[-1]):.4f} | val {np.sqrt(va_hist[-1]):.4f}")
...
class DeepSeekMoE(nn.Module):
    def __init__(self, in_dim, out_dim, hidden=4096, n_exp=8, top_k=2):
        ...
        self.softmax  = nn.Softmax(-1); self.top_k = top_k
        # freeze backbone
...
for fish in fish_list:
    ...
    tr_end, va_end = int(.7*n_frames), int(.8*n_frames)
    ...
    # also save ground-truth tail (per-frame) for convenience
    np.save(os.path.join(fish_dir, f"fish{fish}_groundtruth_full.npy"), Yte)

    # average overlapping windows → per-frame preds
    def overlap_mean(preds, total_len):
        L = preds.shape[1]; out = np.zeros((total_len, preds.shape[2])); cnt = np.zeros(total_len)
    ...
    print(f"Fish {fish} — final test RMSE: {final_rmse:.4f}")
